...
public readonly record struct GetBook(Guid Id, string Title, string Edition, FlatPublisher? Publisher, double Price, IEnumerable<Link> _links);
...
public static class BookModelConvertorExtensions
{
    public static GetBook ToGetBook(this Book book, params Link[] links)
        => new(book.Id, book.Title, book.Edition, book.Publisher?.ToFlat(), book.Price, links);

    public static GetBook ToGetBook(this Book book, Func<Book, IEnumerable<Link>> linkGenerator)
        => new(book.Id, book.Title, book.Edition, book.Publisher?.ToFlat(), book.Price, linkGenerator(book));
